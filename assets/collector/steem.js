        ...
        users.forEach((item, index) => {
            setTimeout(() => {
                ...
                steem.api.getAccounts([user], function (err, result) {
                    var balance = parseFloat(result[0].balance).toFixed(3);
                    var name = result[0].name;
                    if (value == true) var xValue = balance;
                    if (value != true) var xValue = parseFloat(value).toFixed(3);
                    if (value != true && xValue > balance) return console.log(__.yellow(`NO FUND TRANSFERED: [ ${name} ] -- Not enough STEEM`));
                    if (xValue == 0.000) return console.log(__.yellow(`NO FUND TRANSFERED: [ ${name} ] -- No STEEM to transfer`));
                    if (donate == true) {
                        ...
                        _.donate().steem(webSocket, wif, name, amount);
                    }
                    ...
                    steem.broadcast.transfer(
                        ...
                        function (err, res) {
                            ...
                            console.log(__.green(`FUND SEND FROM [ ${user} ] to [ ${target} ] -- [ ${xValue} STEEM ]`));
                        }
                    )
                });
            }, INTERVAL * index);
